...
void config_pins_iomux() {
  // --- SDA Pin : Push-Pull ---
  PORTA.PIN2CTRL &= ~PORT_PULLUPEN_bm;  // pas de pull-up interne
  pinMode(SDA_PIN, OUTPUT);              // push-pull natif AVR

  // --- SCL Pin : Open-Drain émulé ---
  // Règle : OUTPUT+LOW = 0 logique | INPUT (Hi-Z) = 1 logique via résistance externe
  PORTA.PIN3CTRL &= ~PORT_PULLUPEN_bm;  // désactive pull-up interne (résistance externe obligatoire)
  digitalWrite(SCL_PIN, LOW);           // pré-arme LOW pour quand on passe en OUTPUT
  pinMode(SCL_PIN, INPUT);              // état initial = relâché (Hi-Z)
